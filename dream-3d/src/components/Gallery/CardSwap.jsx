...
const makeSlot = (i, distX, distY, total) => ({ // 创建卡片位置槽
  x: i * distX, y: -i * distY, z: -i * distX * 1.5, zIndex: total - i
});

const placeNow = (el, slot, skew) => // 立即放置卡片
...
const CardSwap = ({
  ...
  const config = easing === 'elastic' ? { ease: 'elastic.out(0.6,0.9)', durDrop: 2, durMove: 2, durReturn: 2, promoteOverlap: 0.9, returnDelay: 0.05 } : { ease: 'power1.inOut', durDrop: 0.8, durMove: 0.8, durReturn: 0.8, promoteOverlap: 0.45, returnDelay: 0.2 }; // 动画配置
  ...
  const rendered = childArr.map((child, i) => // 渲染子组件
