...
export default function LiquidEther({
  mouseForce = ANIMATION_CONFIG.LIQUID_BACKGROUND.mouseForce,
  cursorSize = ANIMATION_CONFIG.LIQUID_BACKGROUND.cursorSize,
  isViscous = ANIMATION_CONFIG.LIQUID_BACKGROUND.isViscous,
  viscous = ANIMATION_CONFIG.LIQUID_BACKGROUND.viscous,
  iterationsViscous = ANIMATION_CONFIG.LIQUID_BACKGROUND.iterationsViscous,
  iterationsPoisson = ANIMATION_CONFIG.LIQUID_BACKGROUND.iterationsPoisson,
  dt = ANIMATION_CONFIG.LIQUID_BACKGROUND.dt,
  BFECC = ANIMATION_CONFIG.LIQUID_BACKGROUND.BFECC,
  resolution = ANIMATION_CONFIG.LIQUID_BACKGROUND.resolution,
  isBounce = ANIMATION_CONFIG.LIQUID_BACKGROUND.isBounce,
  colors = ANIMATION_CONFIG.LIQUID_BACKGROUND.colors,
  style = {},
  className = '',
  autoDemo = ANIMATION_CONFIG.LIQUID_BACKGROUND.autoDemo,
  autoSpeed = ANIMATION_CONFIG.LIQUID_BACKGROUND.autoSpeed,
  autoIntensity = ANIMATION_CONFIG.LIQUID_BACKGROUND.autoIntensity,
  takeoverDuration = ANIMATION_CONFIG.LIQUID_BACKGROUND.takeoverDuration,
  autoResumeDelay = ANIMATION_CONFIG.LIQUID_BACKGROUND.autoResumeDelay,
  autoRampDuration = ANIMATION_CONFIG.LIQUID_BACKGROUND.autoRampDuration
}) {
  ...
  useEffect(() => {
    ...
    function makePaletteTexture(stops) { // 创建调色板纹理
      ...
      tex.magFilter = tex.minFilter = THREE.LinearFilter; tex.wrapS = tex.wrapT = THREE.ClampToEdgeWrapping; tex.generateMipmaps = false; tex.needsUpdate = true;
      return tex;
    }

    const paletteTex = makePaletteTexture(colors);
    const bgVec4 = new THREE.Vector4(0, 0, 0, 0); // 透明背景

    class CommonClass { // 通用类
      ...
      init(container) {
        ...
        this.renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true }); this.renderer.autoClear = false; this.renderer.setClearColor(new THREE.Color(0x000000), 0);
      ...
      resize() {
        ...
        this.width = Math.max(1, Math.floor(rect.width)); this.height = Math.max(1, Math.floor(rect.height)); this.aspect = this.width / this.height;
    ...
    class MouseClass { // 鼠标交互类
      constructor() {
        ...
        this._onMouseMove = this.onDocumentMouseMove.bind(this); this._onTouchStart = this.onDocumentTouchStart.bind(this); this._onTouchMove = this.onDocumentTouchMove.bind(this);
        this._onMouseEnter = this.onMouseEnter.bind(this); this._onMouseLeave = this.onMouseLeave.bind(this); this._onTouchEnd = this.onTouchEnd.bind(this);
        this.isHoverInside = false; this.hasUserControl = false; this.isAutoActive = false; this.autoIntensity = 2.0; this.takeoverActive = false; this.takeoverStartTime = 0; this.takeoverDuration = 0.25;
        this.takeoverFrom = new THREE.Vector2(); this.takeoverTo = new THREE.Vector2(); this.onInteract = null;
      }
      init(container) {
        this.container = container;
        container.addEventListener('mousemove', this._onMouseMove, false); container.addEventListener('touchstart', this._onTouchStart, false); container.addEventListener('touchmove', this._onTouchMove, false);
        container.addEventListener('mouseenter', this._onMouseEnter, false); container.addEventListener('mouseleave', this._onMouseLeave, false); container.addEventListener('touchend', this._onTouchEnd, false);
      }
      dispose() {
        if (!this.container) return;
        this.container.removeEventListener('mousemove', this._onMouseMove, false); this.container.removeEventListener('touchstart', this._onTouchStart, false); this.container.removeEventListener('touchmove', this._onTouchMove, false);
        this.container.removeEventListener('mouseenter', this._onMouseEnter, false); this.container.removeEventListener('mouseleave', this._onMouseLeave, false); this.container.removeEventListener('touchend', this._onTouchEnd, false);
      }
      setCoords(x, y) {
        if (!this.container) return; if (this.timer) clearTimeout(this.timer);
        const rect = this.container.getBoundingClientRect(); const nx = (x - rect.left) / rect.width; const ny = (y - rect.top) / rect.height;
        this.coords.set(nx * 2 - 1, -(ny * 2 - 1)); this.mouseMoved = true; this.timer = setTimeout(() => { this.mouseMoved = false; }, 100);
      }
      setNormalized(nx, ny) { this.coords.set(nx, ny); this.mouseMoved = true; }
      onDocumentMouseMove(event) {
        if (this.onInteract) this.onInteract();
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          const rect = this.container.getBoundingClientRect(); const nx = (event.clientX - rect.left) / rect.width; const ny = (event.clientY - rect.top) / rect.height;
      ...
      onDocumentTouchStart(event) { if (event.touches.length === 1) { const t = event.touches[0]; if (this.onInteract) this.onInteract(); this.setCoords(t.pageX, t.pageY); this.hasUserControl = true; } }
      onDocumentTouchMove(event) { if (event.touches.length === 1) { const t = event.touches[0]; if (this.onInteract) this.onInteract(); this.setCoords(t.pageX, t.pageY); } }
      ...
      update() {
        if (this.takeoverActive) {
          ...
          if (t >= 1) { this.takeoverActive = false; this.coords.copy(this.takeoverTo); this.coords_old.copy(this.coords); this.diff.set(0, 0); } else { const k = t * t * (3 - 2 * t); this.coords.copy(this.takeoverFrom).lerp(this.takeoverTo, k); }
        }
    ...
    class AutoDriver { // 自动驱动类
      constructor(mouse, manager, opts) {
        this.mouse = mouse; this.manager = manager; this.enabled = opts.enabled; this.speed = opts.speed; this.resumeDelay = opts.resumeDelay || 3000; this.rampDurationMs = (opts.rampDuration || 0) * 1000;
        this.active = false; this.current = new THREE.Vector2(0, 0); this.target = new THREE.Vector2(); this.lastTime = performance.now(); this.activationTime = 0; this.margin = 0.2; this._tmpDir = new THREE.Vector2(); this.pickNewTarget();
      }
      ...
      update() {
        ...
        const dir = this._tmpDir.subVectors(this.target, this.current); const dist = dir.length(); if (dist < 0.01) { this.pickNewTarget(); return; }
        dir.normalize(); let ramp = 1; if (this.rampDurationMs > 0) { const t = Math.min(1, (now - this.activationTime) / this.rampDurationMs); ramp = t * t * (3 - 2 * t); }
    ...
    class Advection extends ShaderPass { // 平流
    ...
    class ExternalForce extends ShaderPass { // 外力
    ...
    class Viscous extends ShaderPass { // 粘性
    ...
    class Divergence extends ShaderPass { // 散度
    ...
    class Poisson extends ShaderPass { // 泊松
    ...
    class Pressure extends ShaderPass { // 压力
    ...
    container.style.position = container.style.position || 'relative'; container.style.overflow = container.style.overflow || 'hidden';
    ...
    applyOptionsFromProps(); webgl.start();
